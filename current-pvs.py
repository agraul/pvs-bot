...
class RankGetter(object):
    ...
    def _get_linked_account(self,name,author,region):
        ...
        accountLink = [[i for i in line.split(',')] for line in open('linkedAccounts.txt','r')]
...
"""    #Gets rank based on author
    def update_account(self,name,author,region):
        try:
            ...
            return "Error. Account was not properly linked, or the command was mistyped. Make sure to type '!update SUMMONERNAME,REGION'"

    #If the rune page name is correct, save the author's name, summoner name, and region in that order to linkedAccounts.txt
    def link_account(self,name,author,region):
        ...
        if runeName == "Plats vs Silvers":
            ...
            f.close()
            return "Account linked. Type '!update SummonerName,RegionID' to have your rank set"
        else:
            return "Error. Your first rune page must be named 'Plats vs Silvers'. Instead it is '" + runeName + "'"
"""

ranks = ["Bronze","Silver","Gold","Platinum","Diamond","Masters","Challenger"]
...
def verify(message):
    try:
        ...
        region = discord.utils.get(message.server.roles, name=content[1].upper().strip(" "))
        if rank == "Error":
            yield from client.send_message(message.channel, "To verify rank, please set the name of your first rune page to 'Plats vs Silvers'. Make sure you typed the name of your account correctly too.")
        elif rank == "Unranked":
            yield from client.send_message(message.channel, "You are currently not ranked in dynamic queue this season")
        else:
            roles = message.author.roles
            l = [discord.utils.get(message.server.roles,name='Verified'),rank2,region]
            for r in roles:
            ...
            yield from client.send_message(message.channel, "You have been added to {}".format(rank2))
    except:
        yield from client.send_message(message.channel, "Invalid format. Please type '!verify Summoner Name,Region ID'")
"""def link_account(message):
    try:
        ...
        if success == "Account linked. Type '!update SummonerName,RegionID' to have your rank set":
            #role =  #If the account exists and everything succeeds, then add the region as a tag
            try:
                yield from client.add_roles(message.author, discord.utils.get(message.server.roles, name=content[1].upper()))
            except:
                print('Failed')
        yield from client.send_message(message.channel, success)
    except:
        yield from client.send_message(message.channel, "Mistake in format, or summoner not found on that region. Please type '!link SUMMONERNAME,REGIONID'. Type '?!regions' if you do not know your region ID")
...
def update_account(message):
    try:
        ...
        yield from client.remove_roles(message.author, discord.utils.get(message.server.roles, name='Bronze'))
        yield from client.remove_roles(message.author, discord.utils.get(message.server.roles, name='Silver'))
        yield from client.remove_roles(message.author, discord.utils.get(message.server.roles, name='Gold'))
        yield from client.remove_roles(message.author, discord.utils.get(message.server.roles, name='Platinum'))
        yield from client.remove_roles(message.author, discord.utils.get(message.server.roles, name='Diamond'))
        yield from client.remove_roles(message.author, discord.utils.get(message.server.roles, name='Masters'))
        yield from client.remove_roles(message.author, discord.utils.get(message.server.roles, name='Challenger'))
        if success == "Unranked":
            yield from client.send_message(message.channel, "Your account is properly linked, but you aren't ranked in dynamic queue yet")
            yield from client.add_roles(message.author, discord.utils.get(message.server.roles, name='Verified'))
        else:
        ...
        yield from client.send_message(message.channel, "Error. Account was not properly linked, or the command was mistyped. Make sure to type '!update SUMMONERNAME,REGIONID'. Type '?!regions' if you do not know your region ID")
"""
...
def add_coach(message):
    ...
    if x == 2:
        ...
        yield from client.send_message(message.channel, "You need to be at least Platinum to become a coach.")
...
def on_message(message):
    if message.content.startswith('?!roles'):
        yield from client.send_message(message.channel, "Here is a list of available roles:\nRegions: NA, EUW, EUNE, BR, OCE, CHINA, LAN\nRanks: Bronze, Silver, Gold, Platinum, Diamond. (Masters and Challenger must be obtained through the verify method)\nRoles: ADC, Support, Mid, Top, Jungle\nType '?!verify' to learn ow to get the verified tag for your rank")
        ...
        yield from client.send_message(message.channel, "To verify your account, follow these steps:\n1.) Set the name of your first rune page to 'Plats vs Silvers'\n2.) Find your regionID by typing '?!regions'\n3.) Type '!verify SummonerName,RegionID' substituting in your name and region")
    elif message.content.startswith('?!regions'):
        yield from client.send_message(message.channel, "North America = na, EU West = euw, EU North East = eune Latin America North = lan, Latin America South = las, Oceania = oce, Japan = jp, Korea = kr, Brazil = br, Russia = ru, Turkey = tr")
